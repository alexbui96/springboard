...
SELECT f.name AS facility, CONCAT(m.firstname, ' ', m.surname) AS member, ROUND(SUM(b.slots)/2, 1) AS hours_booked
FROM Bookings b
LEFT JOIN Facilities f ON b.facid = f.facid
LEFT JOIN Members m ON b.memid = m.memid
WHERE b.memid <> 0
...
SELECT f.name AS facility, MONTH(starttime) AS month, ROUND(SUM(b.slots)/2, 1) AS hours_booked
FROM Bookings b
LEFT JOIN Facilities f ON b.facid = f.facid
WHERE b.memid <> 0
